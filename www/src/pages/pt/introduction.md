---
title: Introdução
description: Introdução à ShipSpeed Stack
...
A _"ShipSpeed Stack"_ é uma stack de desenvolvimento web feita por [Theo](https://twitter.com/rbnog) focada na simplicidade, modularidade e ser full-stack mantendo a segurança de tipos (typesafe).
...
## Então... o que é o create-shipspeed? Um modelo?

Basicamente. O `create-shipspeed` é uma CLI (Command Line Interface) construída pelos experientes desenvolvedores da ShipSpeed Stack para agilizar a configuração inicial de um aplicativo ShipSpeed modular. Isso significa que cada uma das peças é opcional, e o "modelo" é gerado baseado nas suas necessidades específicas.
...
É fácil cair nessa armadilhas de "adicionar tudo" - e nós explicitamente não queremos fazer isso. Tudo adicionado ao `create-shipspeed` deve resolver um problema específico que existe dentro das principais tecnologias inclusas. Isso significa que nós não iremos adicionar coisas como bibliotecas de gerenciamento de estado (`zustand`, `redux`) mas iremos adicionar coisas como NextAuth.js e integrar o Prisma e tRPC para você.
...
O objetivo declarado do `create-shipspeed` é providenciar a maneira mais rápida de se começar uma aplicação web full-stack e **typesafe**. Nós levamos segurança de tipo muito a sério nessas partes, já que melhora nossa produtividade e nos ajuda a entregar menos bugs. Qualquer decisão que comprometa a natureza de segurança de tipo do `create-shipspeed` é uma decisão que deve ser feita em um projeto diferente.
