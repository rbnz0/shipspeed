...
description: بنية مجلد ShipSpeed
...
يحتوي على الوظائف الخادمة (utilities) التي تقوم بالتوثيق مثل استرداد جلسة المستخدم (user session) من جانب السيرفر. اقرأ [NextAuth.js](https://shipspeed.dev/ar/usage/next-auth#usage-with-trpc) للمزيد من المعلومات.
...
يستخدم ملف `db.ts` ليوضح استخدام Prisma client بشكل شامل. اقرأ [استخدام Prisma](https://shipspeed.dev/ar/usage/prisma#prisma-client) و[افضل الطرق لاستخدام Prisma client مع Next.js](https://www.prisma.io/docs/guides/database/troubleshooting-orm/help-articles/nextjs-prisma-client-dev-practices) لمزيد من المعلومات.
...
1. يحدد المحتوى المستخدم في طلبات tRPC. اقرأ [استعمال tRPC](https://shipspeed.dev/ar/usage/trpc#-serverapitrpcts) للمزيد من المعلومات.

2. يورد (export) نهج الوطيفة المساعدة. اقرأ [استعمال tRPC](https://shipspeed.dev/ar/usage/trpc#-serverapitrpcts) للمزيد من المعلومات.
...
يُستخدم ملف `root.ts` لدمج وظائف tRPC للوصول, وتوريدهم كوظيفة وصول واحدة، وكذلك أنواع بيانات الوظيفة المحددة، اقرأ المزيد هنا [tRPC usage](https://shipspeed.dev/ar/usage/trpc#-serverapirootts).
...
إن ملف `tsconfig.json` ضروري عند استخدام TypeScript. فعيلت بعض الإعدادات الغير افتراضية، مثل `strict mode`، لتوفير أفضل إمكانية للاستخدام الـ TypeScript في ShipSpeed ومكتباتها، لمزيد من المعلومات اقرأ [TypeScript Docs](https://www.typescriptlang.org/docs/handbook/tsconfig-json.html) أو [TypeScript Usage](usage/typescript).
