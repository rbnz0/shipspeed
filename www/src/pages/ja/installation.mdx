---
title: インストール
description: ShipSpeedによるインストール手順
...
`create-shipspeed`を使用してアプリケーションの初期構成を生成するには、以下の 3 つのコマンドのいずれかを実行し、コマンドプロンプトの質問に答えてください：
...
| `-y`, `--default` | CLI の確認をバイパスして、すべてのオプションを選択した新しい ShipSpeed を立ち上げる |
...
以下は、tRPC と Tailwind CSS を使った ShipSpeed の初期構成を生成します。
