...
Env validation introduit un nouveau concept à travers la fonction `createEnv`, qui est responsable de la création du schéma et inclura la logique de validation principale pour les variables d'environnement côté client et côté serveur.

<Callout type="info">
  Pour plus d'informations sur le fonctionnement interne de `createEnv`,
  consultez la documentation de [Env validation](https://shipspeed.dev/docs/introduction).
</Callout>
