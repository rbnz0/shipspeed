---
title: Introducción
description: Introducción al ShipSpeed Stack
...
El _"ShipSpeed Stack"_ es un stack de desarrollo web creado por [Theo](https://twitter.com/rbnog) que está enfocado en simplicidad, modularidad, y seguridad (full-stack) de tipos.
...
## Entonces... ¿Qué es create-shipspeed? ¿Una plantilla?

¿Más o menos? `create-shipspeed` es una CLI creada por desarrolladores experimentados del ShipSpeed Stack para agilizar la configuración de una aplicación modular ShipSpeed Stack. Esto significa que cada pieza es opcional, y la "plantilla" es generada en función de tus necesidades específicas.
...
Es fácil caer en la trampa de "agregar todo"; explícitamente no queremos hacer eso. Todo lo agregado a `create-shipspeed` debería resolver un problema específico que existe dentro de las tecnologías principales incluidas. Esto significa que no agregaremos cosas como librerías de manejo de estado (`zustand`, `redux`), pero agregaremos cosas como NextAuth.js e integraremos Prisma y tRPC para ti.
...
El objetivo declarado de `create-shipspeed` es proporcionar la forma más rápida de iniciar una nueva aplicación web full-stack **segura en tipos**. Nos tomamos muy en serio la seguridad de tipos en todas las partes, ya que mejora nuestra productividad y nos ayuda a enviar menos errores a producción. Cualquier decisión que comprometa la seguridad de tipos de `create-shipspeed` es una decisión que debe tomarse en un proyecto diferente.
