...
description: Getting started with ShipSpeed
...
ShipSpeed uses [@t3-oss/env-nextjs](https://env.t3.gg) along with [zod](https://zod.dev) for validating environment variables at runtime _and_ buildtime. Define your schema in `src/env.js`.
...
The `createEnv` function builds a schema that validates both client and server-side environment variables.

<Callout type="info">
  For more information about how `createEnv` works internally, check out the
  [@t3-oss/env-nextjs docs](https://env.t3.gg/docs/introduction)
</Callout>
