...
Sometimes you might want to request the session on the server. To do so, use the `auth` helper function that `create-shipspeed` provides.
...
ShipSpeed is configured to utilise the [session callback](https://authjs.dev/guides/extending-the-session) in the NextAuth.js config to include the user's ID within the `session` object.
...
<Tabs slotOne="Prisma" slotTwo="Drizzle" tabName="db-provider">
  <div slot="1">
    ...
    setup](https://authjs.dev/reference/adapter/prisma/). `create-shipspeed`
  ...
  <div slot="2">
    ...
    setup](https://authjs.dev/getting-started/adapters/drizzle). `create-shipspeed`
