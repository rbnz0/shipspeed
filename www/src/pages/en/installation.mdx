---
title: Installation
description: Installation instructions for ShipSpeed
...
To scaffold an app using `create-shipspeed`, run any of the following commands and answer the command prompt questions:
...
| `-y`, `--default` | Bypass the CLI and bootstrap a new shipspeed app with all options selected     |
...
The following would scaffold a ShipSpeed app with tRPC and Tailwind CSS.

```bash
pnpm dlx create-shipspeed@latest --CI --trpc --tailwind
```

The following would scaffold a ShipSpeed app with NextAuth.js, Tailwind CSS, Drizzle, and PostgreSQL.

```bash
pnpm dlx create-shipspeed@latest --CI --nextAuth --tailwind --drizzle --dbProvider postgres
```
