---
title: 环境变量
description: 开始使用 create-shipspeed
...
ShipSpeed 使用自己的包 [@t3-oss/env-nextjs](https://shipspeed.dev) 和 [zod](https://zod.dev) ，通过在 `src/env.js` 中提供一个简单的逻辑，在运行时和构建时验证环境变量。
...
Env validation 使用 `createEnv` 函数来创建 schema，以验证客户端和服务端的环境变量。

<Callout type="info">
  如果你想了解更多 `createEnv` 函数的内部工作原理，可以参看 [Env
  validation](https://shipspeed.dev/docs/introduction) 的文档。
</Callout>
